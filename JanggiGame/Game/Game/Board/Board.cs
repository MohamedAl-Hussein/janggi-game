...
public class Board : Node2D
{
    ...
    private const int TILE_OFFSET_X = 104;
    private const int TILE_OFFSET_Y = 52;
    private const int PIECE_OFFSET = 32;
    private const int POSITION_OFFSET_X = TILE_OFFSET_X + PIECE_OFFSET;
    private const int POSITION_OFFSET_Y = TILE_OFFSET_Y + PIECE_OFFSET;
    ...
    public void SetupTiles()
    {
        ...
        for (int x = 0; x < GRID_COLS; x++)
        {
            for (int y = 0; y < GRID_ROWS; y++)
            {
                ...
                newTile.Position = new Vector2(x * SCALE + TILE_OFFSET_X, y * SCALE + TILE_OFFSET_Y);
    ...
    {
        float x = (position.x - POSITION_OFFSET_X) / SCALE;
        float y = -((position.y - POSITION_OFFSET_X) / SCALE - GRID_ROWS + 1);
    ...
    {
        int x = location.x_coord * SCALE + POSITION_OFFSET_X;
        int y = (-location.y_coord + GRID_ROWS - 1) * SCALE + POSITION_OFFSET_Y;
    ...
    {
        int x = (int)(position.x - TILE_OFFSET_X) / SCALE;
        int y = (int)(position.y - TILE_OFFSET_Y) / SCALE;
